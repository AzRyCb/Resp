...
async function handler(conn, msg) {
    ...
    //filter jadbot
    if(m.isGroup && (m.groupMetadata.participants.find(idd => idd.id == '6285722037770@s.whatsapp.net')) && m.user.jadibot) return console.log('Conflict')
    //mute unmute
    ...
    if (attr.isSelf) {
      ...
      console.log(chalk.black(chalk.bgWhite('[ P U B L I C - M O D E ]')) + '\n' + chalk.whiteBright('userName ›'), chalk.green(m.pushName) + '\n' + chalk.whiteBright('command  ›'), chalk.green(logg)) 
        ...
        await conn.sendPresenceUpdate('composing', m)
	 //autoread
